...
if old5 not in s: raise SystemExit('5 mapping not found in base implementation')
...
if old6 not in s: raise SystemExit('* mapping not found in base implementation')
s = s.replace(old6, new6, 1)

old_left = 'if (hit(px, ny, 0.35f, 0.655f, 0.13f, 0.13f)) return Canvas.KEY_LEFT;'
new_left = 'if (px >= 0.18f && px <= 0.49f && ny >= 0.58f && ny <= 0.82f) return Canvas.KEY_LEFT;'
if old_left not in s: raise SystemExit('left mapping not found in base implementation')
